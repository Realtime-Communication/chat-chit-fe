...
function Register() {
    ...
    const submitForm = () => {
        ...
        .then(data => {
            if(data.statusCode === 201) {
                setAlertTag(<Success value={[`Register Success`, 'Back to login page after 3s !']}/>);
                setTimeout(() => {
                    setAlertTag('');
    ...
    const handleFormSubmit = (event) => {
        ...
        if(formData.email && formData.password && formData.name) submitForm();
            ...
            setAlertTag(<Error value={[`Register Fail`, 'Please fullfill significant infomation !']}/>);
            setTimeout(() => {
                setAlertTag('');
            }, 8000)
        }
            ...
            <div className="login-page">
                {alertTag}
                    <form onSubmit={handleFormSubmit} className="form-login">
                        ...
                        <div className="submit_center">
                            <NavLink to="/login" className={'register'}>Back To Login Page</NavLink>
                        </div>
                    </form>
            </div>
        </>
